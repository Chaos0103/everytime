bookstore.sell = {
    ...
    showBookDiv: function ($sellForm, book) {
        ...
        $sellForm.find('div.group-result').remove();
        const $container = $('<div></div>').addClass('group group-book');
        ...
        const $titleWrap = $('<p></p>').appendTo($container);
        const $authorWrap = $('<p></p>').appendTo($container);
        const $publisherWrap = $('<p></p>').appendTo($container);
        const $pubdateWrap = $('<p></p>').appendTo($container);
        const $priceWrap = $('<p></p>').appendTo($container);
        ...
        $('<hr>').appendTo($container);
        const $buttonWrap = $('<p></p>').appendTo($container);
        $('<input>').addClass('button retry').attr({
            type: 'button',
            value: '다시 검색'
        }).appendTo($buttonWrap);
        const $nextButton = $('<input>').addClass('button next disabled').attr({
            type: 'button',
            value: '다음'
        }).appendTo($buttonWrap);
        $('<hr>').appendTo($container);
    ...
    showLectureResultDiv: function ($sellForm, keyword) {
        ...
        const $container = $('<div></div>').addClass('group group-lecture-result').appendTo($sellForm);
        const $loading = $('<div></div>').addClass('loading').text('검색 중입니다...').appendTo($container);
        bookstore.fn.findLectureList('name', keyword, function (response1) {
            bookstore.fn.findLectureList('professor', keyword, function (response2) {
                $container.find('div.loading').remove();
                const $lectureWrap = $('<ol></ol>');
                const lectures = _.chain(response1.concat(response2))
                    .uniq(function (lecture) {
                        return lecture.id;
                    })
                ...
                _.each(lectures, function (lecture) {
                    ...
                    const $lectureLi = $('<li></li>').data('lecture', {id: id, value: value}).appendTo($lectureWrap);
                    $('<h3></h3>').text(name).appendTo($lectureLi);
                    $('<span></span>').addClass('professor').text(professor).appendTo($lectureLi);
                });
                const $resetLi = $('<li></li>').addClass('retry').appendTo($lectureWrap);
    ...
    showStatusImageDiv: function ($sellForm) {
        ...
        const $container = $('<div></div>').addClass('group group-status group-status-image');
        $('<h2></h2>').text('실제 사진을 찍어주세요.').appendTo($container);
        const $images = $('<div></div>').addClass('images').appendTo($container);
        const $imagesWrap = $('<div></div>').addClass('wrap').appendTo($images);
        const imageItems = ['겉표지', '내부 1', '내부 2', '내부 3'];
        _.each(imageItems, function (imageItem) {
            const $image = $('<div></div>').addClass('image').appendTo($imagesWrap);
            $('<span></span>').addClass('remove').text('삭제').appendTo($image);
            $('<span></span>').addClass('icons image-gray-32').appendTo($image);
            $('<p></p>').text(imageItem).appendTo($image); //ㅇㅣ름?
            $('<input>').addClass('file').attr({accept: 'image/*', type: 'file', name: 'file'}).appendTo($imagesWrap);
        });
    ...
    submit: function ($sellForm) {
        ...
        data.append('isbn', $sellForm.find('input[name="book-isbn"]').val());
        data.append('title', $sellForm.find('input[name="book-title"]').val());
        data.append('author', $sellForm.find('input[name="book-author"]').val());
        data.append('publisher', $sellForm.find('input[name="book-publisher"]').val());
        data.append('pubDate', $sellForm.find('input[name="book-pubdate"]').val());
        data.append('price', $sellForm.find('input[name="book-price"]').val());
        if ($sellForm.is(':has(div.group-lecture)')) {
            data.append('lectureId', $sellForm.find('input[name="lecture"]').data('id'));
        }
        data.append('statusNote', $sellForm.find('div.group-status-note > ol.select-checkbox > li').map(function () {
            return $(this).hasClass('checked') ? '1' : '0';
        }).get().join(''));
        data.append('statusDamage', $sellForm.find('div.group-status-damage > ol.select-checkbox > li').map(function () {
            return $(this).hasClass('checked') ? '1' : '0';
        }).get().join(''));
        const $files = $sellForm.find('input[name="file"]');
        for (let i = 0; i < $files.length; i++) {
            data.append('images', $files[i].files[0]);
        }
        data.append('sellPrice', $sellForm.find('input[name="price"]').val());

        if (bookstore.data.defaultLocation) {
            data.append('meansDelivery', 0);
            data.append('meansDirect', 1);
            data.append('meansDirectName', bookstore.data.defaultLocation.name);
        } else {
            data.append('meansDelivery', $sellForm.find('div.group-means > ol.select-checkbox > li:last-child').hasClass('checked') ? 1 : 0);
            data.append('meansDirect', $sellForm.find('div.group-means > ol.select-checkbox > li:first-child').hasClass('checked') ? 1 : 0);
            if ($sellForm.is(':has(div.group-means-direct)')) {
                const meansDirectData = $sellForm.find('input[name="means-direct"]').data();
                data.append('meansDirectName', meansDirectData.name);
            }
        }
        if ($sellForm.is(':has(div.group-comment)')) {
            data.append('comment', $sellForm.find('textarea[name="comment"]').val());
        }
        bookstore.fn.saveItem(data, function (response) {
            location.href = '/book/view/' + response;
            // if (response) {
            //     alert('오류가 발생하였습니다!');
            // } else {
    ...
    uploadS3: function (index, canvas, timestamp, thumb, callback) {
        canvas.toBlob(function (blob) {
            ...
            $.ajax({
                url: 'https://bookstore-item-image.s3.amazonaws.com/',
                type: 'POST',
                data: formData,
                contentType: false,
                processData: false,
...
$().ready(function () {
    ...
    $sellForm
        ...
        .on('click', '> div.group-book input.button.next:not(.disabled)', function () {
            ...
            $buttonWrap.remove();
            bookstore.sell.showLectureDiv($sellForm);
        })
        ...
        .on('change', '> div.group-status-image > div.images input.file', function () {
            ...
            loadImage(file, function (canvas) {
                ...
                bookstore.sell.uploadS3(index, canvas, timestamp, false, function (response) {
            ...
            if (index === 0) {
                loadImage(file, function (canvas) {
                    ...
                    // bookstore.sell.uploadS3(index, canvas, timestamp, true);
