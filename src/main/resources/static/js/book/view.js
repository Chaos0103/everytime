bookstore.view = {
    init: function ($item, response) {
        const data = response;
        const $groupBook = $('<div></div>').addClass('group group-book');
        $('<h1></h1>').html(data.book.title).appendTo($groupBook);
        const $groupBookDl = $('<dl></dl>').appendTo($groupBook);
        ...
        $('<time></time>').html(bookstore.fn.formatDate(data.createdDate)).appendTo($groupItem);
        if (data.soldOut !== 0) {
        ...
        var statusNote = data.statusNote.split('');
        var statusDamage = data.statusDamage.split('');
        ...
        if (data.coverImage || data.images.length) {
            const images = _.compact(_.flatten([data.coverImage, data.images]));
            const $groupStatusImages = $('<div></div>').addClass('images').appendTo($groupStatus);
            const $groupStatusImagesWrap = $('<div></div>').addClass('wrap').width(images.length * 130).appendTo($groupStatusImages);
            _.each(images, function (image) {
                $('<div></div>').addClass('image').css('background-image', 'url("/file/' + image + '")').appendTo($groupStatusImagesWrap);
            });
        ...
        if (data.location) {
            ...
            $('<dd></dd>').html(data.location).appendTo($groupMeansDl);
...
$().ready(function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            bookstore.view.init($item, data);
            bookstore.data.item = data;
        }
    });
    ...
    $item.on('click', '> div.group-item > p.buttons > a.soldout', function () {
            if (!confirm('판매가 완료되면 더이상 쪽지를 받을 수 없습니다. 정말 판매가 완료되었습니까?')) {
                return false;
            }
            var data = {
                itemId: bookstore.data.item.id
            };
        ...
        .on('click', '> div.popup-edit > input.button.submit', function () {
            const $textarea = $item.find('div.popup-edit textarea');
